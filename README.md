...
1. Device Tree (DTS) ConfigurationAdd the following node to your I2C bus section (e.g., &i2c1):
   ```dts
   hynitron@5a {
...
<a name="中文版"></a>
1. 设备树 (DTS) 配置在你的 I2C 总线节点（例如 &i2c1）下添加以下内容：
   ```dts
   hynitron@5a {
    compatible = "hynitron,hyn_ts";
    reg = <0x5a>;
...
3. 参数定义说明属性名说明regI2C 从机地址（十六进制）。reset-gpio复位引脚索引。0x91 对应 145 号引脚。irq-gpio中断引脚索引。0x90 对应 144 号引脚。display-coords十六进制表示的分辨率（宽度, 高度）。3. Debugging / 调试方法查看内核日志:dmesg | grep HYN_TS测试事件上报:getevent -lt
